...
def compute_grpo_outcome_advantage(
    ...
    index: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,  #false是Dr.GRPO
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape is (bs, response_length)
    """
    # 每个 token 的奖励（形状(bs, response_length)），通过sum(dim=-1)在 “响应长度” 维度求和，
    # 得到每个完整响应的总奖励（形状(bs,)，即每个样本一个标量奖励）。
    ## 如果没有计算 token_level_reward，那么token_level_reward=reward_tensor
    print("token_level_rewards:",token_level_rewards)
    scores = token_level_rewards.sum(dim=-1) 
    # 按组（同一个问题）收集奖励
    id2score = defaultdict(list)      #id2score：用字典按index分组
    print("*" * 20)
    print("按组收集奖励 id2score:",id2score)
    print("*" * 20)
    # key 是组ID（来自index数组），value 是该组内所有样本的奖励列表（即同一个问题的不同采样输出的奖励）。
    id2mean = {}
    id2std = {}

    with torch.no_grad():
        bsz = scores.shape[0]
        # 遍历所有样本（i in range(bsz)），将每个样本的奖励scores[i]添加到其对应的组（index[i]）中
        for i in range(bsz):
            print("*" * 20)
            print(i)
            id2score[index[i]].append(scores[i])
            print("id2score[index[i]]:",id2score)
            print("*" * 20)
        print("*" * 20)
        print("每个样本奖励加到对应的组后的 id2score：",id2score)
        #对每个组（idx）计算奖励的均值（id2mean）和标准差（id2std），作为该组的 “基线”：
        #若组内只有 1 个样本（len=1）：均值设为 0，标准差设为 1（避免后续除法出错，且单个样本无参考意义）。
        #若组内有多个样本（len>1）：正常计算均值（平均奖励）和标准差（奖励波动程度）。
        #若组内无样本：抛出错误（无效情况）。
        for idx in id2score:
        ...
        ##优势 =（当前样本奖励 - 组均值）/（组标准差 + epsilon），通过标准差标准化，让优势在不同组间更可比。
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
            else:
                scores[i] = scores[i] - id2mean[index[i]]
        print("计算完 GRPO 后的的 scores：",scores)
        # 响应掩码
